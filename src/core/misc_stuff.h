...
//------------------------------------------------------------------------------------------
#if defined(__ppc__) || defined(__arm64__) || defined(__aarch64__)
...
//------------------------------------------------------------------------------------------
inline float /* > 0 */
exp_interp(float frac /* 0..1 */, float out_min /* > 0 */, float out_max /* > 0 */)
...
struct new_auto_ptr
    : public std::unique_ptr<BASE>
// wrapper for unique_ptr that is initialized with a new instance of "T"
// i.e. instead of writing this:
//     unique_ptr<MyClass> my_class(new MyClass);
// use:
//     new_auto_ptr<MyClass> my_class;

{
  new_auto_ptr()
      : std::unique_ptr<BASE>(new T)
  {
  }
};

//------------------------------------------------------------------------------------------
// allow a unique_ptr to be set to a ptr
template <class A, class B> inline void set(std::unique_ptr<A>& a, B* b)
{
  a.reset(b);
}
...
template <class T> inline Rng<const T> toRng(const Rng<T>& r);
template <class T> inline Rng<T> toRng(T* ptr, int n = 1);
template <class T> inline Rng<T> toRng(std::vector<T>& v);
template <class T> inline Rng<const T> toRng(const std::vector<T>& v);
template <class T> inline Rng<T> toRng(std::valarray<T>& v);
template <class T> inline Rng<const T> toRng(const std::valarray<T>& v);
template <class T, int _SIZE> struct Array;
template <class T, int N> inline Rng<T> toRng(Array<T, N>& src);
template <class T, int N> inline Rng<const T> toRng(const Array<T, N>& src);
template <int LENGTH> class CharArray;
template <int N> inline Rng<char> toRng(CharArray<N>& src);
template <int N> inline Rng<const char> toRng(const CharArray<N>& src);
...
public:
  ...
  // conversion)
  template <class U> Rng(std::vector<U>& v) { *this = ::toRng(v); }
  template <class U> Rng(const std::vector<U>& v) { *this = ::toRng(v); }
  template <class U> Rng(std::valarray<U>& v) { *this = ::toRng(v); }
  template <class U> Rng(const std::valarray<U>& v) { *this = ::toRng(v); }
  template <int N> Rng(Array<T, N>& src) { *this = src.rng(); }
  template <int N> Rng(const Array<T, N>& src) { *this = src.rng(); }
  template <int N> Rng(CharArray<N>& src) { *this = src.rng(); }
  template <int N> Rng(const CharArray<N>& src) { *this = src.rng(); }
  ...
  {
    return idx_within(i, *this) ? base::ptr[i] : out_of_bounds;
  }
...
template <class T> inline Rng<const T> toRng(const Rng<T>& r)
{
  return Rng<const T>(r.ptr, r.n);
}
template <class T> inline Rng<T> toRng(T* ptr, int n)
{
  return Rng<T>(ptr, n);
}
template <class T> inline Rng<T> toRng(std::vector<T>& v)
...
template <class T> inline Rng<const T> toRng(const std::vector<T>& v)
{
  return Rng<const T>(&v[0], (int)v.size());
}
template <class T> inline Rng<T> toRng(std::valarray<T>& v)
{
  return Rng<T>(&v[0], (int)v.size());
...
#else
  int n = vsnprintf((char*)dst.ptr, dst.n, fmt, args);
...
// advantage over normal declaration is that is assignment is allowed
{
  ...
  Rng<const T> rng() const { return toRng((const T*)data, size()); }
  operator Rng<T>() { return rng(); }
  operator Rng<const T>() const { return rng(); }
...
//
{
  ...
  FindClosestArrayElement(ARRAY& values,
                          float /*0..1*/ approx_idxf,
                          float approx_value,
                          float aspect_ratio)
  {
    init();
